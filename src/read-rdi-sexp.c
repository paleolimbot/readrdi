...
SEXP rdi_create_bottom_track(R_xlen_t size) {
    const char* bottom_track_df_names[] = { "magic_number", "bottom_range", "bottom_velocity", "bottom_correlation", "bottom_amplitude", "bottom_pct_good", "bottom_range_msb"  , ""};
...
void rdi_set_bottom_track(SEXP bottom_track_df, R_xlen_t i, rdi_bottom_track_t* bottom_track) {
    ...
    SEXP r_bottom_range = PROTECT(Rf_allocVector(INTSXP, 4));
    for (int j = 0; j < 4; j++) {
        INTEGER(r_bottom_range)[j] = bottom_track->bottom_range[j];
    }
    SET_VECTOR_ELT(VECTOR_ELT(bottom_track_df, 1), i, r_bottom_range);
    UNPROTECT(1);
    SEXP r_bottom_velocity = PROTECT(Rf_allocVector(INTSXP, 4));
    for (int j = 0; j < 4; j++) {
        INTEGER(r_bottom_velocity)[j] = bottom_track->bottom_velocity[j];
    }
    SET_VECTOR_ELT(VECTOR_ELT(bottom_track_df, 2), i, r_bottom_velocity);
    UNPROTECT(1);
    SEXP r_bottom_correlation = PROTECT(Rf_allocVector(INTSXP, 4));
    for (int j = 0; j < 4; j++) {
        INTEGER(r_bottom_correlation)[j] = bottom_track->bottom_correlation[j];
    }
    SET_VECTOR_ELT(VECTOR_ELT(bottom_track_df, 3), i, r_bottom_correlation);
    UNPROTECT(1);
    SEXP r_bottom_amplitude = PROTECT(Rf_allocVector(INTSXP, 4));
    for (int j = 0; j < 4; j++) {
        INTEGER(r_bottom_amplitude)[j] = bottom_track->bottom_amplitude[j];
    }
    SET_VECTOR_ELT(VECTOR_ELT(bottom_track_df, 4), i, r_bottom_amplitude);
    UNPROTECT(1);
    SEXP r_bottom_pct_good = PROTECT(Rf_allocVector(INTSXP, 4));
    for (int j = 0; j < 4; j++) {
        INTEGER(r_bottom_pct_good)[j] = bottom_track->bottom_pct_good[j];
    }
    SET_VECTOR_ELT(VECTOR_ELT(bottom_track_df, 5), i, r_bottom_pct_good);
    UNPROTECT(1);
    SEXP r_bottom_range_msb = PROTECT(Rf_allocVector(INTSXP, 4));
    for (int j = 0; j < 4; j++) {
        INTEGER(r_bottom_range_msb)[j] = bottom_track->bottom_range_msb[j];
    }
    SET_VECTOR_ELT(VECTOR_ELT(bottom_track_df, 6), i, r_bottom_range_msb);
    UNPROTECT(1);
}
